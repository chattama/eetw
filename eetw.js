...
ee.on("tweet", function(tweet) {
    console.log(tweet);
    twitter.post("statuses/update", { status: tweet }, function(error, data, response) {
...
ee.on("twitter.push", function(data) {
    ...
    // 所在地と発生地点の比較
    // 範囲外であればスキップ
    var latlon = { latitude: data.geo.coordinates[1], longitude: data.geo.coordinates[0] };
    console.log(latlon);
...
// twitter stream
twitter.stream("statuses/filter", config.TWITTER_EEW_ACCOUNT, function(stream) {
